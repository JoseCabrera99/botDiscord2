...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === "timear") {
            ...
            const filtroRaw = interaction.options.getString("filtro");
            const filtroRegex = new RegExp(escapeRegex(filtroRaw), 'i'); 
            const MAX_OPTIONS = 25;
            
            try {
                const filteredGraffiti = await Graffiti.find({
                    nombre: { $regex: filtroRegex }
                })
                ...
                if (filteredGraffiti.length === 0) {
                    return interaction.editReply({ 
                        content: `⚠️ No se encontraron grafitis que coincidan con el filtro: **${filtroRaw.toUpperCase()}**.`, 
                    });
                }
                ...
                const selectMenu = new StringSelectMenuBuilder()
                    .setCustomId('timear_grafitti_select') 
                ...
                await interaction.editReply({
                    content: `**Selecciona el graffiti** que deseas timear con el filtro: **${filtroRaw.toUpperCase()}**`,
        ...
        // --- LÓGICA /NEXTGRAFF ---
            ...
            const cutoffTimestamp = nowMs - elevenHoursMs;
            const filteredAndPreparedGraffiti = [];

            try {
                const allGraffiti = await Graffiti.find({
                    nombre: { $regex: filtroRegex },
                    lastSpawnTimestamp: { $lte: cutoffTimestamp } 
                })
                .sort({ numero: 1 })
                .lean();
                ...
                if (allGraffiti.length === 0) {
                    return interaction.editReply({
                        content: `⚠️ No se encontraron grafitis para **${filtroRaw.toUpperCase()}** que hayan pasado el umbral de 11 horas desde su registro.`,
                    });
                }
                ...
                if (filteredAndPreparedGraffiti.length === 0) {
                    await interaction.editReply({
                        content: `⚠️ No se encontraron grafitis para **${filtroRaw.toUpperCase()}** (Filtro post-procesamiento).`,
                    });
                    return;
                }
                ...
                for (let i = 0; i < filteredAndPreparedGraffiti.length; i += CHUNK_SIZE) {
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .setTimestamp()
                        .setFooter({ text: `Filtro: ${filtroRaw.toUpperCase()} | Lote ${chunkNumber} de ${totalChunks}` });
